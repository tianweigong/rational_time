...
cor(sign(dt$rate_gamma)*(abs(dt$rate_gamma))^0.5094413,dt$exp1n2)
...
```{r}
p1=dt %>%
  mutate(x=-sign(rate_gamma)*(abs(rate_gamma))^0.4906598,
         y=-exp1n2)%>%
  mutate(cor="r = .96")%>%
  ggplot(aes(x=x,y=y))+
  ...
  ylab("Human")+
  theme_classic()+
  geom_text(aes(label=cor, size=1),colour="black", x=-2, y=Inf, vjust=1.5, show.legend=FALSE,check_overlap=T)+
