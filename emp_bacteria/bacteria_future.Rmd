...
cor(sign(jud.comb$md)*(abs(jud.comb$md))^1.209465,jud.comb$ans)
cor(jud.comb$md,jud.comb$ans)
cor(jud.comb$md,jud.comb$ans,method = "spearman")
cor(sign(jud.comb$md)*(abs(jud.comb$md))^1.209465,jud.comb$ans,method = "spearman")
```

```{r}
p1=jud.comb %>%
  mutate(x=sign(md)*(abs(md))^1.209465,
         y=ans-4)%>%
  ...
  scale_color_manual(values = alpha(c("#4DAF4A","#984EA3","#FF7F00","#377EB8"),.9),
                     labels=c("Finished",
                              "Unfinished"))+
  scale_shape_manual(values =c(15,16,17,18),labels=c("Finished",
                                                     "Unfinished" ))+
  xlab("Model")+
  ylab("Human")+
  theme_classic()+
  geom_text(aes(label=cor, size=1),colour="black", x=-5.6, y=Inf, vjust=1.5, show.legend=FALSE,check_overlap=T)+
