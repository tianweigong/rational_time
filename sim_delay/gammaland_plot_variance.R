...
x1<-c(60,60,60,60,60) #c(50,55,60,65,70)
...
p1=ggplot(ix, aes(x=k, y=theta, fill=y1))+
  ...
  ggtitle("Five Observations of 60")
...
df.pred.l=df.pred.l %>% 
  mutate(key=factor(key,levels = c("five_obs_of_20","five_obs_of_100"),
                    labels=c("Five Observations of 60","[20, 40, 60, 80, 100]")))
df.aux.ll=df.aux.ll %>% 
  mutate(key=factor(key,levels = c("five_obs_of_20","five_obs_of_100"),
                    labels=c("Five Observations of 60","[20, 40, 60, 80, 100]")))
#Plot everything together
...
ggplot(df.pred.l, aes(x=x, y=y, colour = key))+
  ...
  scale_color_manual(values=alpha(c("#E41A1C","#377EB8"),0.9))
